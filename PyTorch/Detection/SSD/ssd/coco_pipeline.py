...
class DALICOCOIterator(object):
    ...
    def __next__(self):
        ...
        for i in range(self._num_gpus):
            ...
            images_torch_type = to_torch_type[ np.dtype(dali.types.to_numpy_type(images[0].dtype))]
            bboxes_torch_type = to_torch_type[ np.dtype(dali.types.to_numpy_type(bboxes[0][0].dtype))]
            labels_torch_type = to_torch_type[ np.dtype(dali.types.to_numpy_type(labels[0][0].dtype))]
